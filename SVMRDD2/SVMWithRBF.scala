...
class SVMWithRBF(var input: RDD[LabeledPoint], var C: Double, var eps: Double, var tolerance: Double, var gamma: Double) extends Serializable {
  ...
  var b = 0.0 // 阀值
//  var densePoints = Array.ofDim[Double](trainNum, d) // 存放样本的矩阵（old：存放训练与测试样本，0~train_num-1 训练；train_num~N-1 测试）
//  var alpha = new Array[Double](trainNum) // 拉格朗日乘子
  var errorCache = new Array[Double](trainNum) // 存放non-bound样本误差,拉格朗日乘子既非0也非C(non-bound，注意non-bound是针对Ci≤≤α0这一约束条件而言的)
//  var precomputedSelfDotProduct = new Array[Double](N) // 预存dotProductFunc(i,i)的值，以减少计算量
...
//  println(dataMatrix(0)) // 打印第一个点的权值
...
//    ----------------------------------------------------------------------
    ...
    var s = dataMatrix(i1).dot(data)
    s *= -2
    s += precomputedSelfDotProduct(i1) + data.dot(data)
    math.exp(-s * gamma)
  }
